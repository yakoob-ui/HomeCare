...
export default function SearchScreen() {
  ...
  const [{ loading, error, services, pages, countServices }, dispatch] =
    useReducer(reducer, {
      loading: true,
      error: '',
    });
    ...
    const fetchData = async () => {
      try {
        const { data } = await axios.get(
          `/api/services/search?page=${page}&query=${query}&category=${category}&price=${price}&rating=${rating}&order=${order}`
        );
